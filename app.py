...
def chat():
    try:
        ...
        # Check if Gemini actually returned usable text
        # (can be blocked by safety filters, in which case .text raises or is empty)
        try:
            reply_text = response.text
        except Exception:
            # Response was blocked or has no valid text part (e.g. safety filter)
            reason = 'unknown'
            try:
                reason = response.candidates[0].finish_reason.name
            except Exception:
                pass
            return jsonify({"error": f"Response blocked or empty (reason: {reason})"}), 502
        ...
        # Catches quota errors, invalid API key, rate limits, network issues etc.
        return jsonify({"error": str(e)}), 500
